...
function DynamicContentBubble(id, width, height, centered, left, top, title, content,
  ...
  this.content = content.replace(" ", "%20"); // jscript keeps auto unescaping spaces
  ...
  this.close = function(deleteThis, removeFromArray, event) {
    ...
    $(closeA).unbind();
    $(draggerDiv).unbind();
    if ($(verticalDragger).is(".ui-draggable")) {
      $(verticalDragger).draggable("destroy");
    }
    if ($(horizontalDragger).is(".ui-draggable")) {
  ...
  this.resetContents = function(url, title) {
    this.content = url;
    isLoaded = false;
    ...
    $(bubbleBody).load(this.content, function(response, status, xhr) {
      if (status == "success") {
        isLoaded = true;
         ...
         $.ajax({
          type: "POST",
          data: {operation: "content", dcbID: $(bubbleBody).data("theClass").id, url:url},
          url: "services/manageWindows.php"
        });
      }
    });
  ...
  this.scrollToTop = function() {
    if ($(verticalScroll).css("visibility") == "visible") {
  ...
  this.scrollToBottom = function() {
    if ($(verticalScroll).css("visibility") == "visible") {
      ...
      newY -= $(bubbleBody).position().top;
      $(bubbleBody).css("top", newY + "px");
    }
  }
