...
Fill out the form and you will be contacted by email when your account has been added. 
If this page does not submit it is b/c a feild has been left blank or the email address is invalid.
This is not an automated signup, so expect some amount of waiting.<br />
<?
...
<?		if( $_POST["newlogin_name"]!="" && $_POST["newlogin_hash_1"]!="" && $_POST["newlogin_hash_2"]!="" && check_email($_POST["newuser_email"]) )
		{
			if ( $_POST["newlogin_hash_1"]==$_POST["newlogin_hash_2"] )
			{
				...
				if ( check_username($_POST["newlogin_name"]) && check_file($_POST["new_character_name"], $NAMEDENY) && mysql_num_rows($newuser_check)=="0" )
				{
					...
					if ( @mysql_query("INSERT INTO newusers(newuser_group,newlogin_name,newlogin_hash,newuser_email,newuser_browser,newuser_ip) VALUES('player', '" . $_POST["newlogin_name"] . "', '" . $newlogin_hash . "', '" . $_POST["newuser_email"] . "', '" . $_POST["browser"] . "', '" . $_POST["ip"] . "')") )
					{
